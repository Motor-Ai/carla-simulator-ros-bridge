...
void CarlaControlPanel::onInitialize()
{
  ...
  mCarlaStatusSubscriber = _node->create_subscription<carla_msgs::msg::CarlaStatus>("/carla/status", rclcpp::SensorDataQoS().keep_last(10).transient_local(), std::bind(&CarlaControlPanel::carlaStatusChanged, this, _1));
  mCarlaActorListSubscriber = _node->create_subscription<carla_msgs::msg::CarlaActorList>("/carla/actor_list", rclcpp::QoS(rclcpp::QoSInitialization(RMW_QOS_POLICY_HISTORY_KEEP_LAST, 1)).transient_local(), std::bind(&CarlaControlPanel::carlaActorListChanged, this, _1));
  mCarlaControlPublisher = _node->create_publisher<carla_msgs::msg::CarlaControl>("/carla/carla_control", 10);
...
{
  std::string vehicleTopicPrefix = "/carla/";
  ...
  if ( mVehicleTopicPrefix != vehicleTopicPrefix )
  {
    ...
    mVehicleControlStatusSubscriber 
      = _node->create_subscription<carla_msgs::msg::CarlaEgoVehicleStatus>( 
        vehicleTopicPrefix + "/vehicle_status", sensorDataQos, 
...
void CarlaControlPanel::carlaActorListChanged(const carla_msgs::msg::CarlaActorList::SharedPtr msg)
{
  ...
  {
    if ( ( actorInfo.topic_prefix.find("carla/") == 0u ) && (actorInfo.type.find("vehicle.") == 0u) )
    {
      QString const vehicleTopicName(actorInfo.topic_prefix.substr(6).c_str());
...
void CarlaControlPanel::vehicleControlStatusChanged(const carla_msgs::msg::CarlaEgoVehicleStatus::SharedPtr msg)
{
